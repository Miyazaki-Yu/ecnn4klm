import sys
sys.path.append("../")
from ecnn4klm.util.exact import klmtri2d_gc
from ecnn4klm.util.spin_generator import random_spin, rotate_spin_tri2d
...
if __name__ == "__main__":
    ...
    print("#############################")
    sys.stdout.flush()
    #torch._C._jit_set_bailout_depth(0) # remove if eval
    ...
    train_set = data.TensorDataset(spin_train, E_train, B_train) # with E
    train_loader = data.DataLoader(dataset=train_set, batch_size=batch_size, shuffle=True, generator=g)
    test_set = data.TensorDataset(spin_test, E_test, B_test) # with E
    ...
    train_losses = []
    train_losses_E = [] # with E
    train_losses_B = []
    test_losses = []
    test_losses_E = [] # with E
    test_losses_B = []
